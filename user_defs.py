...
async def next_cmd(message: types.Message):
    ...
    message_load = await message.answer(text='Обираю завдання для вас...')
    await task.parsing()
    with open('debug.txt', 'at', encoding='utf-8', newline='\n') as f:
        f.write(f'[{time.ctime()}]: Користувач: {message.from_user.full_name} || Предмет: {task.current_subject} || Завдання: {task.url} ({task.number_task})\n')
