...
def generate_random_7(true_answer):
    done_answers = [true_answer]
    while len(done_answers) < 4:
        nums = random.sample(range(1, 7), 3)
        nums.sort()
        temp_answer = ';'.join(map(str, nums))
        
        if temp_answer not in done_answers:
            done_answers.append(temp_answer)
            
    random.shuffle(done_answers)
    return done_answers